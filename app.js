...
class SUNOPlaylist {
    constructor() {
        ...
        this.elements = {
            ...
            playlistDisplayName: document.getElementById('playlistDisplayName'),
    ...
    init() {
        ...
        this.elements.clearInputBtn?.addEventListener('click', () => this.clearInputFields());
        ...
        this.elements.editBtn?.addEventListener('click', () => this.switchView('input'));
        this.elements.createOwnBtn?.addEventListener('click', () => window.location.href = '/');
        this.elements.playlistDisplayName?.addEventListener('click', () => this.editPlaylistName());
        ...
        this.elements.audioPlayer.addEventListener('volumechange', () => this.updateVolumeIcon());
        ...
        // Initialize view state based on URL params
        const isShared = window.location.pathname.startsWith('/p/') || 
                         new URLSearchParams(window.location.search).has('p') || 
                         new URLSearchParams(window.location.search).has('tracks');
        if (!isShared) {
    ...
    async loadFromShortUrl(shortId) {
        this.showToast(window.i18n && window.i18n.t ? window.i18n.t('toastLoading') : '読み込み中...');
        this.showLoadingProgress(true);
        if (this.elements.progressText) {
            this.elements.progressText.textContent = 'プレイリストを読み込み中...';
        }
        try {
            ...
            if (data.uuids && data.uuids.length > 0) {
                ...
                this.isPlaylistModified = false;
                await this.loadPlaylist(true); // Wait for metadata fetching to complete
                this.showToast(`${data.uuids.length}曲を読み込みました`, 'success');
            } else {
            ...
            if (this.elements.progressText) {
                this.elements.progressText.textContent = 'エラーが発生しました';
            }
        } finally {
    ...
    async loadPlaylist(isFromShortUrl = false) {
        if (!isFromShortUrl) {
            this.currentShortId = null;
            this.isPlaylistModified = true;
        }
        ...
        this.playlistName = this.elements.playlistNameInput ? this.elements.playlistNameInput.value.trim().substring(0, 30) : '';
    ...
    async updateURL() {
        ...
        if (!this.isPlaylistModified && this.currentShortId) {
            let url = `/p/${this.currentShortId}`;
            if (this.playlistName) {
                url += `?name=${encodeURIComponent(this.playlistName)}`;
            }
        ...
        try {
            ...
            console.warn('Failed to save playlist for short URL:', e);
        }
        ...
        let url = `${window.location.pathname}?p=${compressed}`;
        if (this.playlistName) {
            url += `&name=${encodeURIComponent(this.playlistName)}`;
        }
        window.history.replaceState({}, '', url);
    ...
    async getShareUrl() {
        if (!this.isPlaylistModified && this.currentShortId) {
            let shareUrl = `${window.location.origin}/p/${this.currentShortId}`;
            if (this.playlistName) {
                shareUrl += `?name=${encodeURIComponent(this.playlistName)}`;
            }
            return shareUrl;
        }

        try {
            // Use the new API for shorter URLs if available
            const uuids = this.playlist.map(t => t.uuid);
            const response = await fetch('/api/save-playlist', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (response.ok) {
                ...
                this.currentShortId = data.id;
                this.isPlaylistModified = false;
                let shareUrl = `${window.location.origin}/p/${data.id}`;
                if (this.playlistName) {
                    shareUrl += `?name=${encodeURIComponent(this.playlistName)}`;
                }
                return shareUrl;
            ...
            console.warn('Fallback to long URL generation:', e);
            const uuids = this.playlist.map(t => t.uuid).join(',');
            // Compress to save space
            const compressed = LZString.compressToEncodedURIComponent(uuids);
            let shareUrl = `${window.location.origin}${window.location.pathname}?p=${compressed}`;
            if (this.playlistName) {
                shareUrl += `&name=${encodeURIComponent(this.playlistName)}`;
            }
            return shareUrl;
    ...
    async loadFromURL(pParam = null) {
        const urlParams = new URLSearchParams(window.location.search);
        const nameParam = urlParams.get('name') || urlParams.get('n');
        if (nameParam) {
            this.playlistName = nameParam.substring(0, 30);
            if (this.elements.playlistNameInput) {
                this.elements.playlistNameInput.value = this.playlistName;
            }
        } else {
            this.playlistName = '';
            if (this.elements.playlistNameInput) {
        ...
        if (path.startsWith('/p/')) {
            const shortId = path.substring(3).replace(/\/$/, '');
            if (shortId) {
                await this.loadFromShortUrl(shortId);
                return;
            }
        }

        let p = pParam || urlParams.get('p') || urlParams.get('tracks');
            ...
            if (p.includes(',')) {
                ...
                try {
                    ...
                    if (decompressed && decompressed.length > 0) {
                        uuidsString = decompressed;
                    } else {
                        // Fallback: maybe it's just base64 or plain text?
                        uuidsString = p;
                    }
                } catch (e) {
                    console.warn('LZString decompression failed, trying raw:', e);
                    uuidsString = p;
                }
            }

            // Clean up and parse UUIDs
            const uuids = uuidsString.split(',').map(s => s.trim()).filter(s => s);

            if (uuids.length > 0) {
                // Convert UUIDs to Suno URLs
                const links = uuids.map(uuid => {
                    // Handle full URLs or just UUIDs
                    if (uuid.startsWith('http')) return uuid;
                    return `https://suno.com/song/${uuid}`;
                });

                this.elements.linksInput.value = links.join('\n');
                await this.loadPlaylist(); // This will fetch metadata

                // Show success toast
                this.showToast(`${links.length}曲を読み込みました`, 'success');
            } else {
                throw new Error('No valid UUIDs found');
            }
        } catch (error) {
            console.error('Failed to load shared playlist:', error);
            this.showToast('共有リンクの読み込みに失敗しました', 'error');
            this.elements.progressText.textContent = 'エラーが発生しました';
        } finally {
            this.showLoadingProgress(false);
            // Clear URL param to prevent reload loop issues, but keep state clean
    ...
    async shareToTwitter() {
        ...
        const text = window.i18n && window.i18n.t ? window.i18n.t('shareTweetText') : 'Suno AIのプレイリストを作成しました！ 🎧';
        
        const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
        const tweetUrl = `https://x.com/intent/post?url=${encodeURIComponent(url)}&text=${encodeURIComponent(text)}`;
    ...
    updatePlaylistNameDisplay() {
        if (!this.elements.playlistDisplayName) return;
        ...
        if (this.playlistName) {
            this.elements.playlistDisplayName.textContent = this.playlistName;
            this.elements.playlistDisplayName.style.display = 'block';
            this.elements.playlistDisplayName.classList.remove('empty-title');
        } else {
            if (isShared) {
                // Hide for visitors if there's no name
                this.elements.playlistDisplayName.textContent = '';
                this.elements.playlistDisplayName.style.display = 'none';
            } else {
                // Show editable placeholder for the creator
                const untitledText = window.i18n ? window.i18n.t('untitledPlaylist') : 'Untitled Playlist';
                this.elements.playlistDisplayName.textContent = untitledText;
                this.elements.playlistDisplayName.style.display = 'block';
                this.elements.playlistDisplayName.classList.add('empty-title');
            }
        }
    }
    ...
    editPlaylistName() {
        if (!this.elements.playlistDisplayName) return;
        if (this.elements.playlistDisplayName.querySelector('input')) return;

        const currentName = this.playlistName;
        const input = document.createElement('input');
        input.type = 'text';
        input.className = 'playlist-name-edit-input';
        input.value = currentName;
        input.maxLength = 30;
        const placeholderText = window.i18n ? window.i18n.t('playlistNamePlaceholder') : 'Playlist Name';
        input.placeholder = placeholderText;

        this.elements.playlistDisplayName.textContent = '';
        this.elements.playlistDisplayName.appendChild(input);
        input.focus();
        input.select();
        ...
        const saveEdit = () => {
            ...
            if (this.elements.playlistNameInput) {
                this.elements.playlistNameInput.value = newName;
            }
            this.updatePlaylistNameDisplay();
            this.updateURL();
            this.autoSave();
        };
    ...
    setVolume(v) {
        ...
        this.elements.audioPlayer.volume = vol / 100;
    ...
    toggleMute() {
        ...
        if (isMuted) {
            ...
            this.elements.volumeSlider.value = this.previousVolume;
        }
        this.setVolume(this.elements.volumeSlider.value);
    }
    updateVolumeIcon() {
        const isMuted = this.elements.audioPlayer.muted || this.elements.audioPlayer.volume === 0;
        const volUp = this.elements.volumeBtn?.querySelector('.volume-up-svg');
        const volMute = this.elements.volumeBtn?.querySelector('.volume-mute-svg');
    ...
    toggleShuffle() {
        ...
        if (this.shuffleMode) {
            ...
            if (currentTrack) {
                const newIndex = this.playlist.findIndex(t => t.uuid === currentTrack.uuid);
                if (newIndex !== -1) {
                    this.currentIndex = newIndex;
                }
            }
    ...
    clearPlaylist() {
        const confirmMsg = window.i18n && window.i18n.t ? window.i18n.t('confirmClear') : 'Clear?';
    ...
    clearInputFields() {
        if (this.elements.linksInput) {
            this.elements.linksInput.value = '';
            this.elements.linksInput.dispatchEvent(new Event('input', { bubbles: true }));
        }
        if (this.elements.playlistNameInput) {
            this.elements.playlistNameInput.value = '';
        }
        const msg = window.i18n && window.i18n.t ? window.i18n.t('toastInputCleared') : 'Input cleared';
    ...
    restoreLastPlaylist() {
        // Only restore a saved playlist if the URL indicates a shared playlist
        const isShared = window.location.pathname.startsWith('/p/') ||
                         new URLSearchParams(window.location.search).has('p') ||
                         new URLSearchParams(window.location.search).has('tracks');
        if (!isShared) {
            // No shared data – start with a clean session
    ...
    // Clear any existing session data when opening a fresh player (non‑shared)
    clearSession() {
        this.playlist = [];
        this.playlistName = '';
        if (this.elements.playlistNameInput) {
        ...
        const albumArtContainer = document.querySelector('.album-art');
        if (albumArtContainer) albumArtContainer.innerHTML = '🎵';
        if (this.elements.openSunoBtn) this.elements.openSunoBtn.style.display = 'none';
    }
    ...
    switchView(view) {
        ...
        if (view === 'input') {
            ...
            if (this.elements.playerSection) this.elements.playerSection.style.display = 'none';
            if (this.elements.playlistSection) this.elements.playlistSection.style.display = 'none';
        } else if (view === 'player') {
            if (this.elements.inputSection) this.elements.inputSection.style.display = 'none';
            if (this.elements.playerSection) this.elements.playerSection.style.display = 'block';
            if (this.elements.playlistSection) this.elements.playlistSection.style.display = 'block';

            // 共有ボタンと新規作成ボタンのみを表示する
            ...
            if (this.elements.downloadBtn) this.elements.downloadBtn.style.display = 'none';
            if (this.elements.historyBtn) this.elements.historyBtn.style.display = 'none';
            if (this.elements.rankingBtn) this.elements.rankingBtn.style.display = 'none';
            if (this.elements.editBtn) this.elements.editBtn.style.display = 'none';
            if (this.elements.createOwnBtn) this.elements.createOwnBtn.style.display = 'inline-flex';
        }
    }
